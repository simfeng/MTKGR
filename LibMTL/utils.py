...
def count_improvement(base_result, new_result, weight):
    ...
    for task in list(base_result.keys()):
        # improvement += (-1)**np.array(weight[task]) * np.divide(np.array(base_result[task])-np.array(new_result[task]), np.array(base_result[task]), out=np.zeros_like(np.array(base_result[task])), where=np.array(base_result[task])!=0)
