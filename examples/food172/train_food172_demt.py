...
from utils import *
from aspp import DeepLabHead, DeepLabHeadForMultiLabel
from aspp import MultiClassHead, MultiLabelHead
from create_dataset import Food172

from LibMTL import Trainer
from LibMTL.model import swin_b
...
def main(params):
    ...
    def encoder_class(): 
        ...
        return nn.DataParallel(backbone)
    ...
    class Food172Trainer(Trainer):
        ...
        # def on_validation_epoch_end(self,num_samples=32,val_samples=None):
        #     val_images,val_labels = val_samples
        #     # 将 val_samples tensor 导入cpu
        #     val_images=val_images.to(device=self.device)
        #     val_labels=val_labels.to(device=self.device)
        ...
        #                 for x, pred,y in zip(val_images[:self.num_samples],preds[:self.num_samples],val_labels[:self.num_samples])
        #             ]
        #         }
        #     )
