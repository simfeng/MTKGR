...
from utils import *
from examples.food101.head import MultiClassHead
from create_dataset import Food101
...
class Food172Trainer(Trainer):
    def __init__(
        ...
        **kwargs
        ...
        # encoder_class 是个函数
        super(Food172Trainer, self).__init__(
            task_dict=task_dict,
...
def prepare_dataloader(params, test_shuffle=True):
    ...
    if "" in sub_labels:
        sub_labels.remove(
            ""
        )  # [0, 308] # 食材只使用部分标签, [] means all labels, [0, 1, 2] means only use first three labels
...
def prepare_model(params, save_checkpoint_path):
    # set random seed
    ...
    task_dict = {
        "multiclass": {
            ...
            "weight": [
                1,
                1,
            ],  # 0 or 1, 0 means lower is better, 1 means higher is better
        },
        "multilabel": {
            "metrics": [
                "MICRO_PRECISION",
    ...
    print("Food101.num_labels:", Food101.num_labels)
    decoders = nn.ModuleDict(
        {
            "multiclass": nn.DataParallel(MultiClassHead(1000, Food101.num_classes)),
            "multilabel": nn.DataParallel(MultiClassHead(1000, Food101.num_labels)),
        }
    )

    Food172Model = Food172Trainer(
        task_dict=task_dict,
